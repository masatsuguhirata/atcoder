...
//回答url:https://app.codility.com/demo/results/trainingRAMS9Y-YXK/
//
// Task Score: 62%
// Correctness: 75%
// Performance: 50%
//Detected time complexity:計算量は問題無し。回答ミス
...
void eat(vector<int> &v,int i,vector<int> &A,vector<int> &B){
	...
	if(v.size() == 0) v.push_back(i);//比較対象がいない場合は積む
	else if(B[v.size()-1]==1){//食べあう
		if(A[i]>A[v.size()-1]){//下流側の魚のサイズがでかい場合、一つ前の魚と比較する
			v.pop_back();//上流側（で下流に向かう）の魚を取り除く
			eat(v,i,A,B);//もう一つ上流の魚を見にいく
		}
		else{//下流側が小さい場合は、食べられて詰まれない
			// v.push_back(i);
		}
	}
	else v.push_back(i);//下流に向かう魚がいない場合は、上流に向かう魚を積む
	// cout << "2tes" << endl;
}
...
int main(){
	...
	cout << "ans2 ※NGcase:"<< solution(A,B) << endl;}
	return 0;
}
